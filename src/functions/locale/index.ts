 ...
 */
export const loopAndRetrieveTranslations = (
  responseData: IAppWriteResponse
) => {
 ...
 */
export function groupTranslationsByLocale(
  items: IAppWriteDocument[],
  localeFilter?: TLocales
): TGroupedItems {
  return items.reduce((acc, item) => {
    if (localeFilter) {
      acc[item.label_id] = item[localeFilter as TLocales]
    } else {
      acc[item.label_id] = {
        it: item.it,
        en: item.en
