...
import { IAppWriteApi, IAppWritePromise, IAppWriteResponse, TNavigatorLocalePromise } from '@/api/types'
import { AppWriteDB } from '@/database';
import { groupTranslationsByLocale, loopAndRetrieveTranslations } from '@/functions/locale';
...
export const AppWriteApi: IAppWriteApi = {
     ...
     */
    getCollectionFromAppwriteDB: async (databaseId: string, collectionId: string): Promise<IAppWritePromise> => {
        try {
            // Check if the database and collection params exist
            if (!databaseId || !collectionId) throw new Error('Database and collection IDs are required');
            ...
            const results = await AppWriteDB.listDocuments(
                ...
                const loopedData = loopAndRetrieveTranslations(response as IAppWriteResponse)
                if (!loopedData) throw new Error("No Looped Data Can be Found!");
                // Group the AppWrite Looped Data by Locale
                const groupedData = groupTranslationsByLocale(loopedData, locale.language as TLocales);
    ...
    getNavigatorLocale: async (): Promise<TNavigatorLocalePromise> => {
        try {
            ...
            if (!cookieLanguageLocale && !cookieCountryCode) throw new Error("No Cookie Locale Found");
